...
boolean crosshair_enabled = true;
...
void ST_DrawCrosshair(void) {
	...
	int color = 0xFF00FF; // Magenta color

	for (int i = -crosshair_size; i<= crosshair_size; i++) {
		if (i == 1 || i == -1) {
			continue; // Skip the top and bottom of center pixels
		}
		V_DrawPixel(center_x, center_y + i, color); // Vertical line
	}

	for (int i = -crosshair_size; i<= crosshair_size; i++) {
		if (i == 1 || i == -1) {
			continue; // Skip the left and right of center pixels
		}
		V_DrawPixel(center_x + i, center_y, color); // Horizontal line
	}
...
// HUD drawing (crosshair to be drawn before this)
